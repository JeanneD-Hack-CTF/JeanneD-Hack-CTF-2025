...
with open('Fichier_joint_au_message_5_cameras.csv') as csvfile:
...
def memo_pwd_changed(): # (((M+)<(AG<)!)<SN<)<[0-5]
    rang = 0
    for i in CAMERAS_MDP_CHANGE:
        mois = MOIS[rang%len(MOIS)].upper()[::-1]
        lettre_grecque = leetcode(ALPHABET_GREC[rang%len(ALPHABET_GREC)][::-1])
        serial_number = SERIAL_NUMBERS[i-1][::-1]
        res = (mois + lettre_grecque)[::-1]
        res = (res + serial_number)[::-1]
        MOTS_DE_PASSE[i-1] = res
        rang += 1
